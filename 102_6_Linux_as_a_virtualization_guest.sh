#Introduction
 #Virtual machines (VMs) are *simulated* computers. You can use Virtual Machines to create new computers on top of your running machine and install new OSs there. 
 #In some cases, it's also possible to run only  parts of an OS on top of the current OS; This is called having containers.
...
#Type 1 Hypervisor
 #These hypervisors run directly on the host's hardware to control the hardware and manage guest operating systems. For this reason, they are sometimes called bare-metal hypervisors. 
 #The first hypervisors, which IBM developed in the 1960s, were native hypervisors. These included the test software SIMMON and the CP/CMS operating system, the predecessor of  IBM z/VM.
...
#IaaS
 #As the name implies, **Infrastructure as a Service** or IaaS means offloading parts of your infrastructure to other companies. This means buying *services* like electricity, cooling, and even running hypervisors to another company and just renting your VirtualMachine from them. 
 #This makes your life easier because things like "Adding New Hards" now only mean paying a
 #bit more for more HDD on your machine; Instead of buying HDDs and installing them, ... This is called cloud! You might even be able to move your machine from one continent to another one just with one click.
 #Samples of these cloud providers are [Amazon Web Services](https://linux1st.com/(https://aws.amazon.com/)), [Google Cloud Platform](https://linux1st.com/(https://cloud.google.com/)), and [Microsoft Azure](https://linux1st.com/(https://azure.microsoft.com/en-us/)).
