...
You should remember the disk layouts concepts from the [chapter 102.1](https://linux1st.com/1021-design-hard-disk-layout.html). So lets create some paritions using `fdisk`. I will use the `n` for *new*:
...
As seen on [chapter 102.1](https://linux1st.com/1021-design-hard-disk-layout.html), we have used `gdisk` on GPT machines. Its not that different from `fdisk`. Lets have a look at its main commands:
...
The `gparted` tool is a graphical tool to manage your partitioned. It has the ability to resize partitions and is super easy to use. Its not part of the LPIC exam but its good to know about it. 
just in case ;) [gparted.org](https://gparted.org/)

**Formatting the partition**

**Filesystems**

After you partitioned your block devices, you have to format them to make them usable to store files and directories. Formatting a file system, creates a map which stores the location and name of files and directories and make it possible to move files between folders, deleting them or renaming them; think of it as the index of a book.There are several filesystems in the linux world but, these are the most commonly used ones:

| Format | Description |
| --- | --- |
| ext2 | second extended filesystem was developed to address 
shortcomings in the older Unix/Minix filesystem used in early versions 
of Linux. It has been used extensively on Linux for many years. There is
 no journaling in ext2, and it has largely been replaced by ext3 and 
more recently ext4. |
