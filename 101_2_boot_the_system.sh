#The Boot Process
#Important
#You should have a good understanding of what is happening.
...
<aside>
<img src="/icons/code_yellow.svg" alt="/icons/code_yellow.svg" width="40px" /> **UEFI (Unified Extensible Firmware Interface)**
...
The Kernel is the core of your operating system, the LINUX itself. Your bootloader loads the kernel in the memory and runs it. But kernel needs some initial info to start; Things like drivers are necessary to work with the hardware. Those are stored in `initrd` or `initramfs` alongside the kernel and used during the boot. You can also send parameters to the kernel during the boot using the Grub configs. For example, sending a 1 or S will result the system booting in single-user mode (recovery). Or you can force your graphics to work in 1024×768x24 mode by passing `vga=792` to the Kernel during the boot.

<aside>
<img src="/icons/code_yellow.svg" alt="/icons/code_yellow.svg" width="40px" /> **initramfs (Inıtialization Ram File System)**

Kernel's load spesific data system and It has a file which is called initramfs. It’s like a disk in the memory which has some drivers some needed things for the kernel to start. So when the system boots firmwire loads the bootloader, bootloader loads the kernel,and gives it the initramfs so now kernel knows how to control the hardware it can start everything and run everything from here. Inıt runs initializes everything else you need like webservers like network time protocol etc. 

</aside>

**dmesg**

Linux will show you the boot process logs during the boot. Some desktop systems hide this behind a fancy boot splash which you can hide using the Esc key or press Ctrl+Alt+F1. dmesg command will show the full data from **kernel ring buffer** up to now. But `cat /var/log/dmesg` will show only the data during the boot.
...
Is new, loved, and hated. Lots of new ideas but not following some of  the beloved UNIX principles (say.. not saving logs in a text file or trying to help you too much but asking for the root password when you are not running commands with sudo). It lets us run services if the hardware is connected, in time intervals, if nother service is started, and ...
