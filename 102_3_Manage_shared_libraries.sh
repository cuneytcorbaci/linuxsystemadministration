...
#Dynamic library configs and cache
 ...
 #separated list of directories will tell your program where to search for
 #needed libraries **before** checking the libraries in `/etc/ld.so.cache`.For example, if you give this command:
 ...
 #when running a command? Because the executable you are running is a 
 #Linux ELF executable and if you check its inside, you can see that it says "run this using ld-linux": 
