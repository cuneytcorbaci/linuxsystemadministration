#Linking
 #When we write a program, we use libraries. 
 #For example, if you need to read text from standard input, you need to *link* a library that provides this. Think linking has two forms:
 #**Static_Linking** is when you add this library to your executable program. In this method, your program size is big because it has all the needed libraries. One good advantage is your program can be run without being dependent on other programs/libraries.
 #**Dynamic_Linking** is when you just say in your program "We need this and that library to run this program". This way your program is smaller but you need to install those libraries
     #separately. This makes programs more secure (Because libraries can be updated centrally, and more advanced any improvement in a library will improve the whole program), and smaller.
...
#ldd_The `ldd` command helps you find:
...
#Symbolic links for libraries
 #If you are writing a program and you use udev functions, you will ask for a library called *libudev.so.1*. 
 #But a Linux distro, might call its version of udev library *libudev.so.1.4.0*. How can we solve this problem? 
...
#Dynamic library configs and cache
 #As with most other Linux tools, dynamic linking is also configured using a text config file. It is located at */etc/ld.so.conf*. 
 #On an Ubuntu system it just points to other config files in `/etc/ld.so.conf.d/` but all those lines can be included in the main file too:
 ...
 #The `ldconfig` commands processed all these files to make the loading of libraries faster. 
 #This command creates `ld.so.cache` to locate files that are to be dynamically loaded and linked.
...
#Where OS finds dynamic libraries
 ...
 #In these cases, you can point the environment variable LD_LIBRARY_PATH to the library you need to use and then run your program. A colon : 
     #separated list of directories will tell your program where to search for needed libraries **before** checking the libraries in `/etc/ld.so.cache`.
...
#Loading dynamically
 #In the last part of this section, let's see how we can manually tell Linux to run a program using its dynamic linker. Its also called a dynamic loader and is used to load dynamic libraries needed by an executable. 
 #It might be called `ld` or `ld-linux`. You can find yours by running:
 ...
 #Do you want to go deeper than LPIC1 and ask why you do not need to run `ld-linux`
