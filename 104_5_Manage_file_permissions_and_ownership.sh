...
- Each file belongs to one user and one group and this user and the 
members of that group will have specific read/write/execute accesses on 
it. Have a look:
...
- In above example, `jadi` is the owner of the file. The file belongs to the `adm` group and the owner (`jadi` here) has  **r**ead, **w**rite (including deletion and edit) & **e**xecute (reading directory content) permissions on the file while the `adm` group members and **others** only have read & execute access.
    ...
    !https://linux1st.com/images/file_permissions.png
    
    > In many distros, when you create a user, the system creates a group 
    with the same name and assigns that user's files to that group.
    > 
...
- Kindly note the `s` in the *executable bit* for the user permissions and also for the group? That means when any user runs this program, it will be run with the owner of the file access level 
(which is root) instead of that user's id.
